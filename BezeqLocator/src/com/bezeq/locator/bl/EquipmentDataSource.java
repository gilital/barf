...
public class EquipmentDataSource extends LocalDataSource{
	...
	public List<Marker> getMarkers(Context context){
		EquipmentDataManager data = new EquipmentDataManager(context);
		data.open();
		ArrayList<Equipment> list = data.getAllEquipment();
		
		List<Marker> markers = new ArrayList<Marker>();
		for (Equipment equip:list){
			String comment = "Name : " + equip.getName() + "\n"
					+ "Cnum : " + equip.getCnum() + "\n"
					+ "Type : " + equip.getType() + "\n"
					+ "Lat : " + equip.getLatitude() + "\n"
					+ "Long : " + equip.getLongitude() + "\n"
					+ "Alt : " + equip.getAltitude();
			Marker temp = new IconMarker(equip.getName(),comment, equip.getLatitude(), equip.getLongitude(),equip.getAltitude() , Color.DKGRAY, getIcon(equip.getType()));
			markers.add(temp);
		}
	...
	protected void createIcons(Resources res) {
        ...
        icons.put("default", BitmapFactory.decodeResource(res, R.drawable.ic_action_share));
