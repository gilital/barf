...
/**
 * Class for Cabinet table management (local table for showing AR markers only)
 * Methods :
 ...
 *
 */
public class CabinetDataManager extends EquipmentDataManager {

	public CabinetDataManager(Context context) {
		super(context);
	}
	...
	public void insert (int objectID, int merkaz, int featureNum,
		...
		if (equip == null){
			...
			database.insert(DBHelper.CABINET_TABLE_NAME, null, cv);
			Log.i("MSAG_MANAGER","Added new CABINET " + objectID);
		}
		else{
	...
	public void update(int pkid, int objectID, int merkaz, int featureNum,
		...
		database.update(DBHelper.CABINET_TABLE_NAME, cv, DBHelper.PKID_COLUMN + "=" + pkid, null);
		
		Log.i("MSAG_MANAGER","Updating CABINET " + objectID);
	}
	
	public ArrayList<Cabinet> getAll(){
		ArrayList<Cabinet> list = new ArrayList<Cabinet>();
		Cursor cursor = database.query(DBHelper.CABINET_TABLE_NAME,
		        allColumns, null, null, null, null, null);

		    cursor.moveToFirst();
		    while (!cursor.isAfterLast()) {
		    	Cabinet cabinet = new Cabinet(cursor.getInt(0), cursor.getInt(1), cursor.getInt(2), cursor.getInt(3),
	...
	public ArrayList<Cabinet> getInRange(double range) {
		ArrayList<Cabinet> list = new ArrayList<Cabinet>();
		...
		String sql = "SELECT * FROM " + DBHelper.CABINET_TABLE_NAME + " WHERE "
		...
		cursor.moveToFirst();
		while (!cursor.isAfterLast()) {
			Cabinet cabinet = new Cabinet(cursor.getInt(0), cursor.getInt(1),
	...
	public void delete(Cabinet cabinet){
		...
		database.delete(DBHelper.CABINET_TABLE_NAME, DBHelper.PKID_COLUMN + " = " + id, null);
	}
	
	public Cabinet getByObjectID(int objectId){
		Cabinet result = null;
		Cursor cursor = database.rawQuery("SELECT * FROM "+ DBHelper.CABINET_TABLE_NAME +" WHERE "+ DBHelper.OBJECT_ID_COLUMN +" = '"+ objectId +"'", null);
		boolean exists = cursor.moveToFirst();	
		if (exists){
			cursor.moveToFirst();
			result = new Cabinet(cursor.getInt(0), cursor.getInt(1), cursor.getInt(2), cursor.getInt(3),
	    			cursor.getString(4), cursor.getString(5), cursor.getInt(6),
	    			cursor.getString(7), cursor.getDouble(8),cursor.getDouble(9));
	...
	public boolean isEmpty(){
		Cursor cursor = database.rawQuery("SELECT COUNT(*) FROM " + DBHelper.CABINET_TABLE_NAME, null);
